...
def compute_file_hash(file_path: str, algorithm: str = "sha256", chunk_size: int = 8192) -> Optional[str]:
    """
    ...
    try:
        ...
        with open(file_path, "rb") as f:
            if algorithm.lower() in ["autov1", "autov2"]:
                ...
                hasher = hashlib.new(algorithm) if algorithm.lower() in hashlib.algorithms_available else hashlib.sha256()
                while True:
                    data = f.read(chunk_size)
                    if not data:
                        break
...
def compute_file_hashes(file_path: str, algorithms: Optional[list] = None, chunk_size: int = 8192) -> Dict[str, str]:
    """
