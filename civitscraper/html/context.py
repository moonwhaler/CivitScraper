        ...
        for file_path in file_paths:
            ...
            if is_html_file:
                ...
                if model_file_path:
                    ...
                    base_preview_path = os.path.splitext(self.path_manager.get_image_path(model_file_path, "preview0"))[0]
                else:
                    # Fallback to simple naming scheme if model file not found
                    logger.debug(f"Model file not found for HTML, using fallback naming scheme")
            ...
            if is_html_file and preview_rel_path is None:
                ...
                for pattern in additional_patterns:
                    for ext in ['.jpeg', '.jpg', '.png', '.webp', '.mp4']:
                        ...
                        if os.path.isfile(preview_path):
                            preview_rel_path = os.path.relpath(preview_path, output_dir)
                            logger.debug(f"Found preview image with alternative pattern: {preview_path}")
                            logger.debug(f"Preview relative path: {preview_rel_path}")
                            break
                    if preview_rel_path:
                        break
                ...
                if preview_rel_path is None:
                    ...
                    if os.path.isdir(images_dir):
                        ...
                        for filename in os.listdir(images_dir):
                            if model_name.lower() in filename.lower() and any(filename.lower().endswith(ext) for ext in ['.jpeg', '.jpg', '.png', '.webp', '.mp4']):
                                preview_path = os.path.join(images_dir, filename)
                                preview_rel_path = os.path.relpath(preview_path, output_dir)
                                logger.debug(f"Found preview image in images directory: {preview_path}")
                                break
                
                # If still no preview image found, check if there are any in the metadata
                if preview_rel_path is None and 'images' in metadata and isinstance(metadata['images'], list) and metadata['images']:
                    # Get the first image URL from metadata
                    for image in metadata['images']:
                        if 'url' in image and image['url']:
                            preview_url = image['url']
                            logger.debug(f"Using image URL from metadata as fallback: {preview_url}")
